#' Little Falls Safety Dashboard - Final Version
#' 
#' @description
...
#' 2. Detailed breakdown of safety components with enhanced visualizations
#' 3. 7-day flow and safety forecasts with predictive analytics
#' 4. Interactive data visualization tools for comprehensive trend analysis
#' 5. Professional methodology documentation for academic and commercial use
#'
...
# Required packages with explicit loading
library(shiny)
...
#' 
#' @description
#' Creates the user interface for the Little Falls Safety Dashboard with enhanced
#' visualizations, comprehensive methodology, and professional presentation
#'
#' @return A dashboardPage object containing the complete UI definition
#' @noRd
ui <- dashboardPage(
  dashboardHeader(title = "🎓 Little Falls (Potomac) Safety Analysis - Final Version"),
    ...
    tabItems(
        ...
        fluidRow(
          box(
            ...
            wellPanel(
              ...
              tags$div(
                tags$h5("🗓️ Seasonal Factor (20 points max)", style = "color: #e67e22; font-weight: bold;"),
      ...
      tabItem(tabName = "visualization",
        ...
        fluidRow(
          box(
            ...
            wellPanel(
              ...
              tags$ul(
                ...
                tags$li("Trend Stability (30%): Flow change analysis"),
                tags$li("Seasonal Factor (20%): Historical patterns"),
                tags$li("Experience Bonus (10%): Learning conditions")
              )
            ),
          ...
          box(
            title = "📈 Safety Trend Analysis", status = "warning", solidHeader = TRUE,
            width = 6, height = 550,
            wellPanel(
              style = "margin-bottom: 10px; height: 120px; overflow-y: auto;",
              p(tags$strong("Predictive Safety Assessment:"), "7-day safety score trends with risk level forecasting."),
              tags$div(
                style = "display: flex; justify-content: space-around; margin: 5px 0; font-size: 0.85em;",
                tags$span("🟢 Excellent (80+)", style = "color: #27ae60;"),
                tags$span("🟡 Good (60-80)", style = "color: #f39c12;"),
                tags$span("🔴 Poor (<60)", style = "color: #e74c3c;")
              ),
              p(tags$em("Plan trips when forecast shows consistent good scores."), style = "font-size: 0.9em;")
            ),
...
#' 
#' @description
#' Creates the server logic for the Little Falls Safety Dashboard with enhanced
#' error handling, robust data processing, and comprehensive visualizations
#'
#' @param input Shiny input object
#' @param output Shiny output object  
#' @param session Shiny session object
#' @return A function that defines the complete server logic
#' @noRd
server <- function(input, output, session) {
  ...
  #' Update analysis data with enhanced error handling
  #' 
  #' @description
  #' Fetches new data and updates the analysis with comprehensive error handling
  #' 
  ...
  # Enhanced Value Boxes with better error handling
  ...
  output$data_quality <- renderValueBox({
    if (is.null(values$analysis_data)) {
      ...
      # Handle potential NULL or vector data_source values
      if (is.null(source_type) || length(source_type) == 0) {
        source_type <- "unknown"
      } else if (length(source_type) > 1) {
        source_type <- source_type[1]  # Take first element if vector
      }
  ...
  # Enhanced Analysis table with better formatting
  output$analysis_table <- DT::renderDataTable({
    ...
    tryCatch({
      forecast_df <- values$analysis_data$forecast %>%
        mutate(
          Date = date,
          `Flow (cfs)` = round(predicted_flow, 0),
          `Safety Score` = round(safety_score, 0),
  ...
  # Enhanced Safety components plot
  output$safety_components <- renderPlotly({
    ...
    tryCatch({
      scoring_data <- data.frame(
        ...
        Points = c(
          ifelse(is.null(values$analysis_data$detailed_scoring$flow_score), 30, 
                 values$analysis_data$detailed_scoring$flow_score),
          ifelse(is.null(values$analysis_data$detailed_scoring$trend_score), 25, 
                 values$analysis_data$detailed_scoring$trend_score),
          ifelse(is.null(values$analysis_data$detailed_scoring$seasonal_score), 15, 
                 values$analysis_data$detailed_scoring$seasonal_score),
          ifelse(is.null(values$analysis_data$detailed_scoring$experience_score), 5, 
      ...
      p <- ggplot(scoring_data, aes(x = reorder(Component, Points), y = Points, fill = Component)) +
        ...
        coord_flip() +
        scale_fill_manual(values = c("Flow Safety" = "#3498db", "Trend Stability" = "#2ecc71",
                                   "Seasonal Factor" = "#f39c12", "Experience Bonus" = "#9b59b6")) +
  ...
  # Enhanced Time Series Plot for Data Visualization tab
  ...
  # Enhanced Safety Score Breakdown
  ...
  # Enhanced Safety Risk Trend Analysis
...
# Launch message with enhanced formatting
cat("🎓 LAUNCHING LITTLE FALLS SAFETY ANALYSIS - FINAL VERSION\n")
cat("===========================================================\n")
cat("📊 Real-time safety assessment for Little Falls kayaking\n") 
cat("🔬 Site-specific calibration with local expertise\n")
cat("📈 Enhanced visualizations and comprehensive methodology\n")
cat("💻 Access dashboard at: http://localhost:3838\n")
cat("🚀 Final version ready for class presentation and portfolio\n\n")
