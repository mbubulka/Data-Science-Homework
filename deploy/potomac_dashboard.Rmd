...
tryCatch({
  ...
  calculate_safety_score <- function(flow_cfs, trend_cfs = 0) {
    # Handle NA/missing values with robust checks for R 4.3+
    if (length(flow_cfs) == 0 || is.null(flow_cfs) || any(is.na(flow_cfs)) || !is.numeric(flow_cfs)) {
      return(list(
        total = 0,
        flow_score = 0,
        trend_score = 0,
    ...
    # Ensure we are dealing with a single numeric value
    flow_cfs <- as.numeric(flow_cfs[1])
    
    if (length(trend_cfs) == 0 || is.null(trend_cfs) || any(is.na(trend_cfs)) || !is.numeric(trend_cfs)) {
      trend_cfs <- 0
    }
    trend_cfs <- as.numeric(trend_cfs[1])
