...
# Wrap entire setup in try-catch for robustness
tryCatch({
  ...
  calculate_safety_score <- function(flow_cfs, trend_cfs = 0) {
    # Force to scalar numeric and handle any weirdness (NULL, empty, NA, etc)
    flow_val <- suppressWarnings(as.numeric(flow_cfs[1]))
    if (length(flow_val) == 0 || is.na(flow_val)) {
      flow_val <- 2000 # Safety fallback
    }
    
    trend_val <- suppressWarnings(as.numeric(trend_cfs[1]))
    if (length(trend_val) == 0 || is.na(trend_val)) {
      trend_val <- 0 # Safety fallback
    }
    
    # Use explicitly scalar comparisons which are safe in R 4.3+
    if (flow_val < 800) {
      flow_score <- 0
    } else if (flow_val < 1200) {
      flow_score <- 15
    } else if (flow_val < 2000) {
      flow_score <- 35
    } else if (flow_val < 3000) {
      flow_score <- 40
    } else if (flow_val < 4000) {
      flow_score <- 30
    } else {
      flow_score <- 10
    }
    ...
    if (trend_val < -200) {
      trend_score <- 10
    } else if (trend_val < -50) {
      trend_score <- 20
    } else if (trend_val < 50) {
      trend_score <- 30
    } else if (trend_val < 200) {
      trend_score <- 25
    } else {
      trend_score <- 15
    }
  ...
  generate_forecast <- function(base_flow, base_trend = 0) {
    # Validate inputs with robust checks
    if (length(base_flow) == 0 || is.null(base_flow) || any(is.na(base_flow)) || !is.numeric(base_flow)) {
      base_flow <- 2000
    } else {
      base_flow <- as.numeric(base_flow[1])
    }
    
    if (length(base_trend) == 0 || is.null(base_trend) || any(is.na(base_trend)) || !is.numeric(base_trend)) {
      base_trend <- 0
    } else {
      base_trend <- as.numeric(base_trend[1])
    }
  ...
  # Ensure we have valid numeric data (robust to R 4.3+ stricter if behavior)
  if (is.null(data_result$flow_cfs) || length(data_result$flow_cfs) == 0 || any(is.na(data_result$flow_cfs))) {
    data_result$flow_cfs <- 2000  # Default fallback value
  }
  if (is.null(data_result$trend_cfs) || length(data_result$trend_cfs) == 0 || any(is.na(data_result$trend_cfs))) {
    data_result$trend_cfs <- 0  # Default fallback value
  }
  if (is.null(data_result$timestamp)) {
    data_result$timestamp <- Sys.time()
  }
  if (is.null(data_result$data_source)) {
    data_result$data_source <- "unknown"
  }
  ...
  # Emergency fallback if entire setup fails
  cat("Setup error:", as.character(e), "\n")
  ...
  forecast_data <<- data.frame(
    day = 1:7,
    predicted_flow = seq(2000, 2600, length.out = 7),
    safety_score = rep(50, 7),
...
- **Data Source:** `r if(data_result$data_source == "USGS_live") "🟢 USGS Live" else "🟡 Demo Mode"`

**Last Updated:** `r format(current_conditions$timestamp, "%Y-%m-%d %H:%M")`

---

**🔗 Part of bubulkaanalytics.com**  
*Analytics Dashboard Portfolio*

Column {data-width=600}
-----------------------------------------------------------------------

### 📈 7-Day Flow & Safety Forecast
